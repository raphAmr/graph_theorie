...
public class Arbre {
    ...
    public void addNode(int node) {
        if (!critique.containsKey(node)) {
            critique.put(node, new ArrayList<Integer>());
        }
    }
    ...
    public void dfs(int node, List<Integer> path, List<List<Integer>> paths,int fin) {
        ...
        for (int pred : critique.get(node)) {
            path.add(pred);
